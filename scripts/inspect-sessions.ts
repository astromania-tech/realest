  ...
  for (const r of roles) {
    try {
      ...
      const token = await loadSupabaseAccessToken({
        ...
        emailEnvNames: [
          r.label === 'ADMIN' ? 'ADMIN_EMAIL' : r.label === 'AGENT' ? 'AGENT_EMAIL' : r.label === 'OWNER' ? 'OWNER_EMAIL' : 'USER_EMAIL',
        ],
        passwordEnvNames: [
          r.label === 'ADMIN' ? 'ADMIN_PASSWORD' : r.label === 'AGENT' ? 'AGENT_PASSWORD' : r.label === 'OWNER' ? 'OWNER_PASSWORD' : 'USER_PASSWORD',
        ],
      })
      ...
      try {
        ...
        console.warn('Failed decode jwt payload for', r.label, e.message)
      }
      ...
      if (sessionId) {
        try {
          const sessRes = await fetch(`${base}/rest/v1/auth.sessions?id=eq.${sessionId}`, { headers })
          const sessText = await sessRes.text()
          console.log(r.label, 'auth.sessions:', sessText)
        } catch (e) {
          console.log(r.label, 'auth.sessions query failed:', e.message)
        }
      } else {
        console.log(r.label, 'No session_id in token')
      }

        // Verify session with auth.v1 user endpoint using the freshly minted access token
        try {
          ...
          console.log(r.label, '/auth/v1/user check failed:', e.message)
        }
      // Best-effort: query recent audit entries for this actor
      try {
        const param = `${encodeURIComponent('payload->>actor_username')}=eq.${encodeURIComponent(r.email)}`
        const auditRes = await fetch(`${base}/rest/v1/auth.audit_log_entries?select=payload,created_at&${param}&order=created_at.desc&limit=10`, { headers })
        const auditText = await auditRes.text()
        console.log(r.label, 'auth.audit_log_entries (recent):', auditText)
      } catch (e) {
        console.log(r.label, 'audit query failed:', e.message)
      }
    } catch (e) {
      console.error(r.label, 'error:', e.message)
    }
  }
}

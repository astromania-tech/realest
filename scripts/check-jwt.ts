...
function formatExpiry(epoch) {
  ...
  return `${d.toISOString()} (in ${Math.round((d - Date.now()) / 1000)}s)`
}
...
for (const c of candidates) {
  ...
  if (result.valid) {
    ...
    console.log(`${c.name}: INVALID (${result.reason}) — ${result.expiresAt ? 'expired at ' + formatExpiry(result.expiresAt) : ''}`)
  }
}

// Refresh token check: look for common env names
const refreshNames = ['SUPABASE_REFRESH_TOKEN', 'REFRESH_TOKEN', 'REALEST_REFRESH_TOKEN']
const found = refreshNames.map(n => ({ name: n, value: process.env[n] })).filter(x => x.value)
if (found.length === 0) {

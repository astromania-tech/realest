...
-- =============================================================================
-- STEP 2: Backfill public.users from profiles (+ optional user_roles)
-- Role priority:
--   1) user_roles.role (if table exists)
--   2) profiles.candidate_role (if exists + valid)
--   3) 'user'
-- =============================================================================
DO $$
BEGIN
  IF to_regclass('public.user_roles') IS NOT NULL THEN
    ...
    SELECT
      ...
      COALESCE(
        ur.role,
        CASE
          WHEN p.candidate_role = ANY (ARRAY['user','agent','owner','admin']::text[])
            THEN p.candidate_role::public.user_role
...
-- =============================================================================
-- Drop existing intra-schema FK if it already exists
ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_users_id_fkey;

-- Also drop the old cross-schema one if present
ALTER TABLE public.profiles
...
-- =============================================================================
DROP FUNCTION IF EXISTS public.safe_to_regrole(public.user_role);
DROP TABLE  IF EXISTS public.user_role_to_db_role;
...
-- =============================================================================
-- STEP 12: Enable RLS on public.users + baseline policies (idempotent)
-- =============================================================================
ALTER TABLE public.users ENABLE ROW LEVEL SECURITY;

-- Always drop first to make re-runs safe
...
-- Public: owner profiles readable
...
-- Drop existing policies to make Step 13 rerunnable
DROP POLICY IF EXISTS "profiles_insert_self" ON public.profiles;
DROP POLICY IF EXISTS "admins_view_audit_logs" ON public.admin_audit_log;
DROP POLICY IF EXISTS "Admins can manage agents" ON public.agents;
DROP POLICY IF EXISTS "Admins can manage KYC requests" ON public.kyc_requests;
